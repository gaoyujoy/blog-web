...
onMounted(async () => { try { const [list, tags] = await Promise.all([postsApi.list({ page: 1, limit: 100 }), tagsApi.list()]); posts.value = list.data.items; tagNames.value = tags.data.map(item => item.name); } catch { posts.value = []; tagNames.value = []; } });
</script>
<template>
