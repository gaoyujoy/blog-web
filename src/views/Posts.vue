...
const escapeReg = (value: string) =>
  value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
const highlight = (value: string) =>
  keyword.value.trim()
    ? value.replace(
        new RegExp(`(${escapeReg(keyword.value.trim())})`, "ig"),
...
watch(keyword, () => {
  ...
  timer = window.setTimeout(() => fetchPosts(true), 300);
});
const onScroll = () => {
  if (
    sentinel.value &&
...
<template>
  <section class="list-heading">
    <div class="eyebrow">// all notes</div>
    <h1>文章</h1>
    <p>把踩过的坑、做过的实验和仍在思考的问题，写成可检索的记录。</p>
  </section>
  <div class="toolbar">
    <input
      v-model="keyword"
  ...
  <section class="post-list">
    ...
    >
      ...
      <h2 v-html="highlight(post.title)" />
      <p v-html="highlight(resultText(post))" />
      <div class="tags">
