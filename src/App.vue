...
const active = computed(() =>
  route.path.startsWith("/posts") ? "/posts" : route.path,
);
function toggleTheme() {
  dark.value = !dark.value;
  document.documentElement.classList.toggle("dark", dark.value);
  localStorage.setItem("coder-theme", dark.value ? "dark" : "light");
}
  ...
  <div class="app-shell">
    <header class="site-header">
      <RouterLink to="/" class="brand">
        ...
        ><span>gaoyu<span class="muted">.dev</span></span>
      </RouterLink>
      <nav>
        <RouterLink to="/" :class="{ active: active === '/' }">
          首页 </RouterLink
        ><RouterLink to="/posts" :class="{ active: active === '/posts' }">
          文章 </RouterLink
        ><RouterLink to="/archive" :class="{ active: active === '/archive' }">
          归档
        </RouterLink>
      </nav>
